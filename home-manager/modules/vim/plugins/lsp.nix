    ...
    config = ''
        ...
        lsp.on_attach(function(_, bufnr)
            ...
            map({'n', 'x'}, 'gq',
                ...
                {desc = 'Format Buffer/Selection'}, bufnr)
            map('n', 'K', function()
                local winid = require('ufo').peekFoldedLinesUnderCursor()
                if not winid then vim.lsp.buf.hover() end
            end, {desc = 'Show hover menu'}, bufnr)
            map('n', '<LocalLeader>p', peekDefinition, {desc = 'Peek definition'}, bufnr)
            map('n', '<LocalLeader>h',
            ...
            map('n', '<LocalLeader>cl', vim.lsp.codelens.run, {desc = 'Run Codelens'},
                bufnr)
        end)
        ...
        lsp.setup_servers({'bashls', 'dockerls', 'rnix', 'terraformls', force = true})
